...
```{r}
sum(is.na(baked_data$logQmean))   # Count missing values
sum(is.infinite(baked_data$logQmean))  # Count infinite values
...
camels_recipe <- recipe(logQmean ~ p_mean + pet_mean + elev_mean + area_gages2 + soil_porosity + slope_mean, 
  ...
  step_normalize(all_predictors())  # Normalizes predictors for better model performance
